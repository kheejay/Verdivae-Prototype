<template>
    <header class="w-screen h-fit border sticky top-0 bg-white p-4 flex justify-between z-[2]">
        <div class="flex gap-0">
            <MascotLogo class="h-[6rem]" />
            <LogoAsName class="h-[6rem] -translate-x-8" />
        </div>
        <div class="flex items-center gap-6 pr-2.5">
            <input type="text" class="min-w-[24rem] h-[3.5rem] rounded-full bg-empty focus:outline-none text-[1.5rem] py-4 px-8">
            <NoProfile @click="$router.push({ name: 'Profile' })" class="h-[3.5rem] cursor-pointer active:scale-95 duration-200" />
            <Burger @click="showSideBar" class="cursor-pointer active:scale-95 duration-200 h-[3rem]" />
        </div>
    </header>
